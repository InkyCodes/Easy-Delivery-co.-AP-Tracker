...
easy_delivery_co__location = {}
easy_delivery_co__location.__index = easy_delivery_co__location
...
---@type table<string, easy_delivery_co__new_return>
NAMED_LOCATIONS = {}
local stale = true
...
---@class easy_delivery_co__new_return
...
---@field name string
---@field side string?
---@field exits table<integer, {[1]:easy_delivery_co__new_return, [2]:fun(): accessibilityLevel}>
---@field keys integer
...
---@param name string
---@return easy_delivery_co__new_return
function easy_delivery_co__location.new(name)
	if name == nil then
		error("easy_delivery_co__location name cannot be nil")
	end
	if NAMED_LOCATIONS[name] then
		print("easy_delivery_co__location " .. name .. " already exists")
	end
    local self = setmetatable({}, easy_delivery_co__location)
	self.name = name
...
---@param exit string|easy_delivery_co__new_return easy_delivery_co__new_return or code/name
---@param rule? fun(): accessibilityLevel|boolean
function easy_delivery_co__location:connect_one_way(exit, rule)
    if type(exit) == "string" then
        ...
        if existing then
            ...
            exit = easy_delivery_co__location.new(exit)
        end
    end
    if rule == nil then
        rule = always
...
---@param exit string|easy_delivery_co__new_return easy_delivery_co__new_return or code/name
---@param rule? fun(): accessibilityLevel|boolean
function easy_delivery_co__location:connect_two_ways(exit, rule)
...
---@param exit string|easy_delivery_co__new_return easy_delivery_co__new_return or code/name
---@param rule? fun(): accessibilityLevel|boolean
function easy_delivery_co__location:connect_one_way_entrance(name, exit, rule)
    if rule == nil then
        rule = always
    end
    self.exits[#self.exits + 1] = { exit, rule }
...
---@param exit string|easy_delivery_co__new_return easy_delivery_co__new_return or code/name
---@param rule? fun(): accessibilityLevel|boolean
function easy_delivery_co__location:connect_two_ways_entrance(name, exit, rule)
...
function easy_delivery_co__location:connect_two_ways_entrance_door_stuck(name, exit, rule1, rule2)
...
---@param rule1? fun(): accessibilityLevel|boolean
---@param rule2? fun(): accessibilityLevel|boolean
function easy_delivery_co__location:connect_two_ways_stuck(exit, rule1, rule2)
...
---@return accessibilityLevel
function easy_delivery_co__location:accessibility()
...
---@param keys integer
function easy_delivery_co__location:discover(accessibility, keys)
...
Entry_Point = easy_delivery_co__location.new("Entry_Point")
